...
function setupInitialViewListeners() {
    ...
    if(textInput) {
        textInput.addEventListener('input', (e) => {
            setState({ pastedText: (e.target as HTMLTextAreaElement).value });
        });
    }
    ...
    fileUploadInput.addEventListener('change', (e) => handleFile((e.target as HTMLInputElement).files[0]));
    ...
    // Style Guides
    const styleGuideInput = document.getElementById('style-guide-upload') as HTMLInputElement;
    ...
    const handleStyleFiles = async (files: FileList) => {
    ...
    if (styleGuideInput) {
        styleGuideInput.addEventListener('change', e => handleStyleFiles((e.target as HTMLInputElement).files));
        ['dragenter', 'dragover', 'dragleave', 'drop'].forEach(eventName => styleGuideDropZone.addEventListener(eventName, e => e.preventDefault()));
        styleGuideDropZone.addEventListener('drop', e => {
            e.preventDefault();
            handleStyleFiles((e as DragEvent).dataTransfer.files)
        });
    }
    ...
    document.getElementById('numQuestions').addEventListener('input', e => {
        ...
        document.getElementById('num-questions-label').textContent = value.toString();
    ...
    document.querySelectorAll('.question-type-cb').forEach(cb => {
        cb.addEventListener('change', e => {
            ...
            const type = target.dataset.questionType;
...
function setupQuizListeners() {
    document.getElementById('question-card-container').addEventListener('click', e => {
        const button = (e.target as HTMLElement).closest('.mc-option-btn');
        if (button) {
            const answer = (button as HTMLElement).dataset.answer;
...
function setupResultsListeners() {
    ...
    downloadBtn.addEventListener('click', async () => {
        ...
        try {
            const canvas = await html2canvas(pdfContentContainer.firstElementChild as HTMLElement, { scale: 2 });
...
function setupHistoryModalListeners() {
    ...
    document.querySelectorAll('.history-load-btn').forEach(btn => {
        btn.addEventListener('click', e => {
            ...
            const isReattempt = target.dataset.isReattempt === 'true';
